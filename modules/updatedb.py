...
def updatedb(phenny, input):
    if input.admin:
        ...
        conn = connect(host=c[cfg]['host'], user=c[cfg]['user'], passwd=c[cfg]['passwd'], db=c[cfg]['db'])
        curs = conn.cursor()
        ...
        for item in d.entries:
            ...
            if t > 1:
                m = match(r'^\[[\w|-]+\] (.+) - (\d\d[v\d]*) \[([0-9A-F]{8})\]\.mkv$', item.title)
                n = match(r'^.+loads - ([\d|\.]{1,6} \wiB) - .+$', item.description)
                if m == None:
                    fail += '{} '.format(item.title)
                else:
                    i += 1
                    command += "('{}', '{}', '{}', '{}', '{}'), ".format(m.group(1), m.group(2), m.group(3), item.guid, n.group(1))
                    if i == 1:
                        new = item.published
        ...
        if len(fail) > 0:
            reply += ' | Please add manually: {}'.format(fail)
        return phenny.reply(reply)
updatedb.rule = (['updatedb'], r'(.*)')
updatedb.priority = 'high'
